...
export function Sidebar() {
  ...
  return (
    <aside className="glass flex h-full w-80 shrink-0 flex-col border-r border-border-soft">
      <div className="px-5 pb-2 pt-5">
        <p className="section-label mb-3.5">Sites</p>
        <SiteSwitcher />
      </div>

      <div className="mx-5 my-2.5 h-px bg-border-soft" />

      <div className="flex min-h-0 flex-1 flex-col px-5 pb-3">
        <p className="section-label mb-3 shrink-0">Boards</p>
        <BoardList />
      </div>

      <nav className="flex flex-col gap-1.5 border-t border-border-soft p-4">
...
function NavButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'group relative flex items-center gap-3 overflow-hidden rounded-xl px-3.5 py-3 text-sm transition-colors',
    ...
    >
      ...
      />
      <Icon size={18} strokeWidth={2} className={cn('transition-colors', active && 'text-accent')} />
